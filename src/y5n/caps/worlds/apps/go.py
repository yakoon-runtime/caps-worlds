...
async def main():
    ...
    boxes = ports.get("worlds.box.service")
    ...
    if ref == "..":
        ...
        await session.update(patch={"data": {"worlds.current_box": box.parent_id}})
    ...
    if child is not None:
        await session.update(patch={"data": {"worlds.current_box": child.id}})
        await io.write(f"{child.name}")
        return

    endpoints = ports.get("worlds.endpoint.service")
    connections = ports.get("worlds.connection.service")
    ...
    patch = {"worlds.current_box": target.id}
    if target_world != current_world:
        patch["worlds.current_world"] = target_world
    await session.update(patch={"data": patch})

    if target_world != current_world:
        worlds = ports.get("worlds.world.service")
