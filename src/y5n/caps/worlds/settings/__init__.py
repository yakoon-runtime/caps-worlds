...
class Settings:
    ...
    def load(cls, config_dir: Path | None = None) -> Settings:
        data = resolve_space_config("worlds", config_dir)
        return cls(
            storage=StorageSettings(
                backend=_backend(
                    os.getenv("WORLDS_STORE_BACKEND")
                    or data.get("storage", {}).get("backend"),
                    "memory",
                ),
                dsn=os.getenv("WORLDS_STORE_DSN")
                or data.get("storage", {}).get("dsn")
                or "",
            ),
            sequencer=SequenceSettings(
                backend=_backend(
                    os.getenv("WORLDS_SEQUENCER_BACKEND")
                    or data.get("sequencer", {}).get("backend"),
                    "memory",
                ),
                dsn=os.getenv("WORLDS_SEQUENCER_DSN")
